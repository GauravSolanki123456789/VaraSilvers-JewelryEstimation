...
async function initSchema() {
    const queries = [
        ...
        `CREATE INDEX IF NOT EXISTS idx_products_secondary_image_url ON products(secondary_image_url) WHERE secondary_image_url IS NOT NULL`,
    ...
    try {
        ...
        // Optional secondary product image (KC / B2B)
        await pool.query(`
            DO $$
            BEGIN
                IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                               WHERE table_name='products' AND column_name='secondary_image_url') THEN
                    ALTER TABLE products ADD COLUMN secondary_image_url TEXT;
                END IF;
            END $$;
        `);

        // Gifting products: fixed selling price (KC sync → fixedPrice)
        await pool.query(`
            DO $$
            BEGIN
                IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                               WHERE table_name='products' AND column_name='fixed_price') THEN
